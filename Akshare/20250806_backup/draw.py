...
def draw_prediction_vs_actual(stock_code, years=5, model_path=None, ax=None):
    ...
    scaler_path = os.path.join(config.MODEL_DIR, "scalers.joblib")
    scalers = joblib.load(scaler_path)
    daily_scaler, weekly_scaler, monthly_scaler = scalers['daily'], scalers['weekly'], scalers['monthly']
    ...
    daily_featured[config.FEATURE_COLUMNS['daily']] = daily_scaler.transform(daily_featured[config.FEATURE_COLUMNS['daily']])
    weekly_featured[config.FEATURE_COLUMNS['weekly']] = weekly_scaler.transform(weekly_featured[config.FEATURE_COLUMNS['weekly']])
    monthly_featured[config.FEATURE_COLUMNS['monthly']] = monthly_scaler.transform(monthly_featured[config.FEATURE_COLUMNS['monthly']])
...
def draw_all_best_models(stock_code, years=1):
    ...
    axes = axes.flatten() # 将2D数组转换为1D，方便迭代

    for i, model_path in enumerate(sorted(model_paths)):
        if i < len(axes):
            draw_prediction_vs_actual(stock_code, years, model_path=model_path, ax=axes[i])
