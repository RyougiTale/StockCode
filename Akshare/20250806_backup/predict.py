...
def predict_for_date(stock_code, target_date_str):
    ...
    if not os.path.exists(scaler_path):
        ...
        print("Please run the training script (train.py) successfully at least once to generate the scaler file.")
        return
    ...
    # b. 数据归一化 (正确方式：使用加载的scaler进行transform)
    daily_df_featured[config.FEATURE_COLUMNS] = daily_scaler.transform(daily_df_featured[config.FEATURE_COLUMNS])
    weekly_df_featured[config.FEATURE_COLUMNS] = weekly_scaler.transform(weekly_df_featured[config.FEATURE_COLUMNS])
    monthly_df_featured[config.FEATURE_COLUMNS] = monthly_scaler.transform(monthly_df_featured[config.FEATURE_COLUMNS])
