...
def calculate_actual_3d_metrics(df, look_forward_days=20):
    ...
    for i in range(len(df) - look_forward_days):
        ...
        # 总收益率
        total_return = (future_prices[-1] - future_prices[0]) / future_prices[0]
    ...
    for index, row in tqdm(target_df.iterrows(), total=len(target_df), desc=f"预测 {stock_code}"):
        ...
        with torch.no_grad():
            ...
            if 'label_generator' in locals():
                ...
                # 回退到基准中心点
                return_centers = np.array([-0.15, -0.05, 0.02, 0.08, 0.20])    # 收益率基准
                sharpe_centers = np.array([-1.0, 0.0, 0.5, 1.0, 2.0])         # 夏普比率基准
                drawdown_centers = np.array([-0.25, -0.15, -0.08, -0.04, -0.01]) # 回撤基准
            ...
            # 计算不同Top-K预测
            # Top-1预测
            return_top1 = return_centers[np.argmax(return_probs)]
            sharpe_top1 = sharpe_centers[np.argmax(sharpe_probs)]
            drawdown_top1 = drawdown_centers[np.argmax(drawdown_probs)]
            ...
            return_top3 = np.sum(return_top3_probs_norm * return_centers[return_top3_idx])
            ...
            sharpe_top3 = np.sum(sharpe_top3_probs_norm * sharpe_centers[sharpe_top3_idx])
            ...
            drawdown_top3 = np.sum(drawdown_top3_probs_norm * drawdown_centers[drawdown_top3_idx])
...
def draw_3d_long_term_comparison(stock_code, model_path=None, years=3, save_path=None):
    ...
    if model_path is None:
        ...
        if model_path is None:
            raise FileNotFoundError("未找到可用的3D模型文件")
    ...
    if not df_historical.empty:
        ...
        ax1.plot(df_historical['date'], df_historical['pred_return_top3'] * 100, 
                 label='历史预测 (Top-3)', color='darkviolet', linewidth=2, linestyle='--')
        ax1.plot(df_historical['date'], df_historical['pred_return_top1'] * 100, 
                 label='历史预测 (Top-1)', color='red', linewidth=2, linestyle=':')
    ...
    if not df_future.empty:
        ...
        ax1.plot(df_future['date'], df_future['pred_return_top3'] * 100, 
                 label='未来预测 (Top-3)', color='plum', linewidth=2, linestyle='--')
        ax1.plot(df_future['date'], df_future['pred_return_top1'] * 100, 
                 label='未来预测 (Top-1)', color='lightcoral', linewidth=2, linestyle=':')
...
def print_performance_statistics(df, stock_code, model_name):
    ...
    for metric, name in zip(metrics, metric_names):
        ...
        if actual_col in df.columns:
            corr_full = df[actual_col].corr(df[pred_full_col])
            corr_top3 = df[actual_col].corr(df[pred_top3_col])
            corr_top1 = df[actual_col].corr(df[pred_top1_col])
            
            rmse_full = np.sqrt(np.mean((df[actual_col] - df[pred_full_col])**2))
            rmse_top3 = np.sqrt(np.mean((df[actual_col] - df[pred_top3_col])**2))
            rmse_top1 = np.sqrt(np.mean((df[actual_col] - df[pred_top1_col])**2))
            
            print(f"\n🎯 {name}:")
            print(f"  相关系数 - 全概率: {corr_full:.4f} | Top-3: {corr_top3:.4f} | Top-1: {corr_top1:.4f}")
            print(f"  RMSE     - 全概率: {rmse_full:.4f} | Top-3: {rmse_top3:.4f} | Top-1: {rmse_top1:.4f}")
