...
def main():
    ...
    #     logger.info(f"限制样本数量为500用于测试，原样本数: {len(all_samples)}")
    #     all_samples = all_samples[-500:]  # 取最新的500个样本
...
def save_best_3d_models(model, val_losses, val_accs, best_models, save_dir, epoch):
    ...
    """
    min_epoch_to_save = 1  # 测试时从第1个epoch开始保存
    min_acc_threshold = 0.05  # 降低阈值
...
if __name__ == '__main__':
    ...
    if config.TRAINING_PHASE != "pretraining":
        logger.error(f"当前训练阶段为 '{config.TRAINING_PHASE}'，请在config.py中设置为'pretraining'")
        exit(1)
