...
import pandas as pd
try:
    from .label_3d_generator import ThreeDimensionalLabelGenerator, get_label_generator
    from . import config
...
class Market3DClassificationDataset(Dataset):
    ...
    def __init__(self, samples, look_forward_days=20, temperature=0.002, use_relative_metrics=True, stock_distributions=None):
        """
        Args:
        ...
        self.soft_labels_3d = []
        invalid_count = 0
        self.raw_metrics = [] # 用于存储原始指标值以供分析
        
        for i, sample in enumerate(self.samples):
            # 从future_prices计算三个指标
            metrics = None
            if 'future_prices' in sample and len(sample['future_prices']) > 1:
                ...
                # 计算三个指标
                metrics = self.label_generator.calculate_future_metrics(future_prices)
            
            self.raw_metrics.append(metrics) # 存储原始指标
            
            # 生成3D软标签
            if metrics:
                stock_code = sample.get('stock_code', None)
                label_3d = self.label_generator.create_soft_label_3d(metrics, stock_code)
            else:
                # 如果没有足够的future_prices，使用均匀分布
                label_3d = {
            ...
            # 验证标签有效性
            if self._validate_3d_label(label_3d):
                self.soft_labels_3d.append(label_3d)
            else:
                # 使用均匀分布作为备选
    ...
    def _validate_3d_label(self, label_3d):
        ...
        for key, label in label_3d.items():
            ...
            if (label < 1e-8).any():
                return False
            if abs(label.sum().item() - 1.0) > 1e-6:
                return False
        return True
