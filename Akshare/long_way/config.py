...
if TRAINING_PHASE == "pretraining":
    ...
    # STOCK_CODES = ['002001', '002463', '002648', '600489', '601989', '000425', '002422', '600150', '600584', '601166']
...
# 3D标签系统配置（替代旧的1D SOFT_LABEL_CONFIG）
SOFT_LABEL_CONFIG = {
    "LOOK_FORWARD_DAYS": 20,
    "TEMPERATURE": 0.1  # 3D标签系统的温度参数
}
...
PRETRAINING_BATCH_SIZE = 64   # 降低批次大小，减少梯度累积
PRETRAINING_LEARNING_RATE = 0.00001  # 大幅降低学习率防止梯度爆炸

# 微调阶段配置  
FINETUNING_EPOCHS = 1000       # 微调轮数可以适当减少，因为有预训练基础
FINETUNING_BATCH_SIZE = 32    # 微调用较小批次，更精细
...
GRAD_CLIP_NORM = 1.0  # 降低梯度裁剪阈值，更严格控制梯度

# 模型保存路径配置
# 使用相对于 long_way 目录的路径
# 获取当前配置文件所在目录（long_way目录）
LONG_WAY_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_DIR = os.path.join(LONG_WAY_DIR, "models")
