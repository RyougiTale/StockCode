...
def _get_config_hash():
    """获取影响数据生成的配置的哈希值，用于缓存验证"""
    import hashlib
    
    # 将影响数据生成的关键配置序列化
    config_items = [
        config.SOFT_LABEL_CONFIG["LOOK_FORWARD_DAYS"],
        config.DAILY_SEQ_LEN,
        config.WEEKLY_SEQ_LEN, 
        config.MONTHLY_SEQ_LEN,
        str(config.FEATURE_COLUMNS),
        config.TRAINING_YEARS,
    ]
    
    config_str = str(config_items)  # 不需要排序，直接转字符串
...
def create_samples_for_code(code):
    """为单只股票/指数创建所有样本（新版：软标签），带缓存优化"""
    
    # 缓存文件路径
    cache_dir = os.path.join(config.MODEL_DIR, "data_cache")
    os.makedirs(cache_dir, exist_ok=True)
    cache_file = os.path.join(cache_dir, f"{code}_samples.pkl")
    ...
    if os.path.exists(cache_file):
        try:
            ...
            # 检查缓存版本和配置是否匹配
            cache_config = cached_data.get('config_hash')
            current_config = _get_config_hash()
            
            if cache_config == current_config:
                logger.info(f"使用股票 {code} 的缓存数据")
                return cached_data['samples'], cached_data['scalers']
            else:
                logger.info(f"股票 {code} 缓存配置过期，重新计算")
        except Exception as e:
            logger.warning(f"读取股票 {code} 缓存失败: {e}")
    ...
    try:
        ...
        logger.info(f"股票 {code} 数据已缓存")
    except Exception as e:
        logger.warning(f"保存股票 {code} 缓存失败: {e}")
    
    return final_samples, scalers
