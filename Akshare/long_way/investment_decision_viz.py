...
class InvestmentDecisionVisualizer:
    ...
    def predict_stock(self, stock_code, days=30):
        ...
        Args:
            stock_code: 股票代码
            days: 回看历史天数（用于分析趋势，模型固定预测未来20天）
        ...
        for index, row in tqdm(target_df.iterrows(), total=len(target_df), desc=f"预测 {stock_code}"):
            ...
            with torch.no_grad():
                ...
                # 提取3D预测结果 - 这些是未来20天的表现等级概率（模型输出已经是log_softmax）
                ...
                # 使用改进标签生成器的实际自适应中心点
                # 这些是实际的绝对性能指标，不是归一化后的相对评分
                
                # 获取当前股票的自适应中心点（如果没有分布数据，会使用基准中心点）
                return_centers = np.array(self.label_generator.get_adaptive_centers(self.current_stock_code, 'total_return'))
                sharpe_centers = np.array(self.label_generator.get_adaptive_centers(self.current_stock_code, 'sharpe_ratio'))  
                drawdown_centers = np.array(self.label_generator.get_adaptive_centers(self.current_stock_code, 'max_drawdown'))
                
                # 计算未来20天的预期表现
                expected_return_20d = np.sum(return_probs * return_centers)      # 预期20天收益率
                expected_sharpe_20d = np.sum(sharpe_probs * sharpe_centers)      # 预期20天夏普比率
                expected_drawdown_20d = np.sum(drawdown_probs * drawdown_centers) # 预期20天最大回撤
                ...
                def calculate_confidence(probs):
                    ...
                    # 最大熵为log(5)≈1.609，将其转换为0-1的信心度
                    max_entropy = np.log(5)
                    confidence = 1 - (entropy / max_entropy)
                    return confidence
                
                return_confidence = calculate_confidence(return_probs)
                sharpe_confidence = calculate_confidence(sharpe_probs) 
                drawdown_confidence = calculate_confidence(drawdown_probs)
                ...
                predictions.append({
                    ...
                    'expected_sharpe_20d': expected_sharpe_20d,
    ...
    def create_investment_dashboard(self, stock_code, days=30, save_path=None):
        ...
        fig.add_trace(
            go.Scatter(
                x=pred_df['date'],
                y=pred_df['expected_return_20d'] * 100,  # 转换为百分比
                name='预期20天收益率(%)',
        ...
        fig.add_trace(
            go.Scatter(
                x=pred_df['date'],
                y=pred_df['expected_drawdown_20d'] * 100,  # 转换为百分比
                name='预期20天最大回撤(%)',
    ...
    def _generate_investment_advice(self, predictions):
        ...
        recent_avg_return = np.mean([p['expected_return_20d'] for p in predictions[-5:]])  # 最近5天平均
        
        # 综合评分 - 基于未来20天预期表现（实际绝对指标）
        # 收益率: 正值越大越好，范围大约-15%到+20%
        # 夏普比率: 正值越大越好，范围大约-1.0到+2.0
        # 最大回撤: 负值越小越好（接近0更好），范围大约-25%到-1%
        score = (
            (latest['expected_return_20d'] + 0.15) / 0.35 * 0.4 +  # 收益率归一化到[0,1]
            (latest['expected_sharpe_20d'] + 1.0) / 3.0 * 0.3 +    # 夏普比率归一化到[0,1] 
            (latest['expected_drawdown_20d'] + 0.25) / 0.24 * 0.2 + # 回撤归一化到[0,1]
    ...
    def _print_investment_summary(self, stock_code, predictions):
        ...
        print(f"\n🎯 未来20天预测指标:")
        print(f"  📈 预期收益率: {latest['expected_return_20d']*100:.2f}% (信心度: {latest['return_confidence']:.3f})")
        print(f"  📊 预期夏普比率: {latest['expected_sharpe_20d']:.3f} (信心度: {latest['sharpe_confidence']:.3f})")  
        print(f"  📉 预期最大回撤: {latest['expected_drawdown_20d']*100:.2f}% (信心度: {latest['drawdown_confidence']:.3f})")
        print(f"  🎲 综合预测信心度: {latest['overall_confidence']:.3f}")
        print(f"\n📝 指标说明: 这些是基于AI模型预测的未来20天绝对性能指标")
        
        # 计算趋势
        if len(predictions) > 1:
            trend_return = latest['expected_return_20d'] - predictions[-2]['expected_return_20d']
            trend_symbol = "📈" if trend_return > 0 else "📉" if trend_return < 0 else "➡️"
            print(f"  📊 收益预期趋势: {trend_symbol} {trend_return*100:+.2f}%")
...
def main():
    ...
    # 获取回看天数（用于趋势分析）
    try:
        days = int(input("请输入回看历史天数 (默认30天，仅影响趋势分析，预测固定为未来20天): ") or "30")
    except ValueError:
        days = 30
