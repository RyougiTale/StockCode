...
# 旧的全局中心点计算函数已被移除，因为新的改进系统使用自适应中心点策略
...
def main():
    ...
    # --- 3b. 微调阶段：加载预训练模型 ---
    if config.TRAINING_PHASE == "finetuning" and config.FINETUNING_CONFIG.get("use_pretrained_model", True):
        pretrained_model_path = os.path.join(config.MODEL_DIR, config.PRETRAINING_MODEL_NAME)
    ...
    for epoch in range(config.EPOCHS):
        ...
        # 保存最终模型
        save_best_3d_models(model, val_losses, val_accs, best_models, config.MODEL_DIR, epoch + 1)
    ...
    # 选择损失最小的模型进行测试，或者使用最终模型
    best_loss_models = sorted(best_models['total_loss'], key=lambda x: x[0])
    best_model_path = best_loss_models[0][1] if best_loss_models[0][1] else final_model_path
